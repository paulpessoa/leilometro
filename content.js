...
;(function () {
  ...
  const SITE_SELECTORS = {
    'leilo.com.br': {
      lance:       ['[class*="lance"] [class*="valor"]', '[class*="bid"] [class*="amount"]', '.current-bid', '.valor-atual'],
      ano:         ['[class*="ano"]', '[data-ano]', 'td:contains("Ano")', '.detalhe-veiculo td'],
      km:          ['[class*="km"]', '[class*="odometro"]', '.mileage', '[data-km]'],
      combustivel: ['[class*="combust"]', '[class*="fuel"]', '.combustivel'],
      modelo:      ['[class*="titulo"]', 'h1', '.vehicle-title', '.nome-veiculo'],
    },
    'vipleiloes.com.br': {
  ...
  function scrapePageData() {
    ...
    // Extração inteligente — também vasculha texto geral da página se falhar
    let lance = extractNumber(lanceRaw);
    let ano   = extractYear(anoRaw) || extractYear(document.body.innerText);
    let km    = extractKM(kmRaw)    || extractKM(document.body.innerText);
    ...
    else if (pageText.includes('kit gás') || pageText.includes('kit gas') || pageText.includes('gnv')) combustivel = '🟡 Flex + GNV';
    ...
    else if (pageText.includes('álcool') || pageText.includes('alcool') || pageText.includes('etanol')) combustivel = '🌿 Etanol';
    ...
    if (!lance) {
      ...
      if (matches) {
        ...
        if (valores.length) lance = Math.min(...valores); // menor valor visível = provável lance
      }
    }

    const modelo = modeloRaw ? modeloRaw.replace(/\s+/g, ' ').substring(0, 60) : 'Veículo não identificado';
    const idadeVeiculo = ano ? (anoAtual - ano) : null;
    const kmAno = (km && idadeVeiculo && idadeVeiculo > 0) ? Math.round(km / idadeVeiculo) : null;

    return { lance, ano, km, kmAno, combustivel, modelo, idadeVeiculo };
  }
  ...
  function calcularOportunidade(dados) {
    const { lance } = dados;
    const fipe = CONFIG.fipeMock;

    const comissao     = lance ? lance * CONFIG.comissaoLeiloeiro : 0;
    const custoReal    = lance ? lance + comissao + CONFIG.taxaPatio : 0;

    const custosFixos  = CONFIG.custoDocumentacao + CONFIG.custoIPVA +
                         CONFIG.custoConserto + CONFIG.taxaPatio;
    ...
    const potencialLucro  = lance ? (fipe - custoReal - CONFIG.custoConserto - CONFIG.custoDocumentacao - CONFIG.custoIPVA) : null;
    const score           = calcularScore(lance, precoLimite, dados.kmAno);

    return { comissao, custoReal, precoLimite, lancesRestantes, potencialLucro, score, fipe };
  }
  ...
  function createPanel(dados, calc) {
    ...
    panel.innerHTML = `
      ...
      <div class="li-body">
        ...
        <div class="li-grid">
          <div class="li-item">
            <span class="li-item-label">FIPE Referência</span>
            <span class="li-item-value">${BRL(calc.fipe)}</span>
          </div>
          <div class="li-item">
        ...
        <div class="li-grid">
          ...
          <div class="li-item">
            <span class="li-item-label">Combustível</span>
            <span class="li-item-value">${dados.combustivel}</span>
          </div>
          <div class="li-item li-item-full">
            <span class="li-item-label">Intensidade de Uso</span>
  ...
  function observeLanceChanges() {
    ...
    try { target = document.querySelector(allSels); } catch (_) {}
